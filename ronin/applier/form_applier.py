...
class FormApplier:
    ...
    def apply_ai_response(self, element_info: Dict, ai_response: Dict, driver):
        ...
        try:
            ...
            logger.debug(f"Applied {element_type} response")
    ...
    def _apply_checkbox_response(self, element, ai_response: Dict, driver):
        ...
        desired_ids = set(ai_response.get("selected_options", []))

        # Debug: log what AI returned and what checkboxes exist
        available_ids = [cb.get_attribute("id") for cb in checkboxes]
        ...
        for checkbox in checkboxes:
            ...
            if is_selected != should_be_selected:
                logger.debug(
                    f"Clicking checkbox {checkbox_id} (was: {is_selected}, want: {should_be_selected})"
                )
                checkbox.click()
